...
# ------------------------------------------------------
# Default Routine (ALWAYS USED)
# ------------------------------------------------------
DEFAULT_ROUTINE = [
    {"label": "Wake Up", "start": "07:00", "end": "07:30"},
    {"label": "Breakfast", "start": "08:00", "end": "08:30"},
    {"label": "Lunch", "start": "13:00", "end": "14:00"},
    {"label": "Evening Break", "start": "17:00", "end": "17:30"},
    {"label": "Dinner", "start": "20:00", "end": "20:30"},
...
def get_free_slots(for_date, routine, existing_tasks, start_datetime):
    ...
    busy = merge_busy_slots(routine, existing_tasks.get(for_date, []))
...
# ------------------------------------------------------
# MAIN PLANNER (always default routine)
# ------------------------------------------------------
def generate(req: PlannerRequest) -> PlannerResponse:

    USER_ROUTINE = DEFAULT_ROUTINE   # ⭐ ALWAYS USE DEFAULT
    ...
    buckets = {start_date + timedelta(days=i): [] for i in range(8)}
    ...
    for score, task, remaining, due_dt in scored:
        ...
        while remaining > 0 and current_day <= end_date:
            ...
            free = get_free_slots(current_day, USER_ROUTINE, existing, start_dt)
    ...
    schedule = [
        ...
        for d, blocks in buckets.items() if blocks
    ]
